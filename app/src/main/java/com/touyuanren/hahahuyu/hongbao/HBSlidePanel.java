...
public class HBSlidePanel extends ViewGroup {
    private List<HBItemView> viewList = new ArrayList<HBItemView>(); // 存放的是每一层的view，从顶到底
    ...
    private List<HBDataItem> dataList; // 存储的数据链表
    ...
    public HBSlidePanel(Context context) {
        this(context, null);
    }

    public HBSlidePanel(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public HBSlidePanel(Context context, AttributeSet attrs, int defStyle) {
    ...
    protected void onFinishInflate() {
        ...
        for (int i = num - 1; i >= 0; i--) {
            ...
            if (childView.getId() == R.id.card_bottom_layout) {
                ...
                HBItemView viewItem = (HBItemView) childView;
        ...
        HBItemView bottomCardView = viewList.get(viewList.size() - 1);
    ...
    private class DragHelperCallback extends ViewDragHelper.Callback {
        ...
        public void onViewPositionChanged(View changedView, int left, int top,
                                          int dx, int dy) {
            onViewPosChanged((HBItemView) changedView);
        }
        ...
        public boolean tryCaptureView(View child, int pointerId) {
            ...
            ((HBItemView) child).onStartDragging();
            boolean shouldCapture = ((HBItemView) child).shouldCapture(downPoint.x, downPoint.y);
            if (shouldCapture) {
        ...
        public void onViewReleased(View releasedChild, float xvel, float yvel) {
            animToSide((HBItemView) releasedChild, (int) xvel, (int) yvel);
        }
    ...
    public void onViewPosChanged(HBItemView changedView) {
    ...
    private void orderViewStack() {
        ...
        HBItemView changedView = (HBItemView) releasedViewList.get(0);
        ...
        for (int i = num - 1; i > 0; i--) {
            HBItemView tempView = viewList.get(i);
            tempView.setAlpha(1);
        ...
        if (newIndex < dataList.size()) {
            HBDataItem dataItem = dataList.get(newIndex);
     ...
     * 松手时处理滑动到边缘的动画
     */
    private void animToSide(HBItemView changedView, int xvel, int yvel) {
     ...
     */
    public void fillData(List<HBDataItem> dataList) {
        this.dataList = dataList;

        int num = viewList.size();
        for (int i = 0; i < num; i++) {
            HBItemView itemView = viewList.get(i);
